...
The current lifecycle state schema is v7 and uses the full stage names above. It records local-update attempt fingerprints, mutation-free reconciliation outcomes, and verified update proofs without absolute installation paths. Entering `local_update` requires the exact publication proof, preview, and local-update approval. Apply reserves a fingerprint once before mutation; `completed` requires a verified update proof. A retry transition is legal only after a recorded `not_applied` or `rolled_back` outcome and always needs a new preview and confirmation.

Legacy Preview states are migrated deterministically before current-schema validation when every required proof can be preserved. A known active remote-action or local-update state whose required proof cannot be inferred is blocked with recovery guidance; unknown versions or phases are blocked instead of guessed.
