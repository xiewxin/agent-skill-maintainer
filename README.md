...
> **Preview:** local analysis, isolated candidate implementation, release gates, and confirmed GitHub PR, merge, and Release apply are implemented. Branch push, local Skill update, and the complete live lifecycle are still being validated.
...
- **Prepare and control publication** with separate previews and confirmations for supported PR, merge, and Release actions.
...
| Publication | State-bound previews and separate confirmations for each supported GitHub write |
...
isolated implementation → tests → complete Diff review
        ↓
separately confirmed PR → merge → release
```
...
GitHub actions use three separate steps: create a state-bound preview, create an expiring approval only after explicit confirmation, then apply it from the matching active run. The lifecycle consumes the approval first; apply records a one-time attempt before re-checking the active account, permission, base and head commits, and branch or PR:
...
- read-only recovery or absence proof for PR, merge, or Release after an interrupted apply;
...
- Git branch push and local Skill update;
...
- Raw conversations, secrets, personal data, private source code, and local process files do not belong in the public repository.
