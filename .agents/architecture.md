...
Read-only GitHub inspection derives and fingerprints the active account, permission, default branch, relationship, immutable-Release setting, and Release capability. Action previews bind that proof and apply re-reads live state. Lifecycle completion is explicit; a verified stop after merge may seed a new bounded Release continuation. A migrated pre-v8 terminal merge may use the same entrypoint only after its detached proof, exact remote Pull Request changed-file set, merged tree, and unique target Skill identity match, as defined by [ADR 0010](adr/0010-bound-legacy-remote-skill-identity.md). The optional `publish_pr` action combines only exact branch push and initial PR creation, retaining non-replayable branch proof while unobservable PR state stays pending; only proven absence unlocks granular fallback.

Publishable same-model A／B evidence uses randomized labels, an independent pre-unblind zero-tool Judge, provider transcripts constrained by the locked read-only tool profile, and exact private-source rederivation. Post-Judge platform evidence is bound to an external authority through a short-lived signed challenge and signed post-execution completion evidence; Codex and Claude transcript formats are provider-specific. The final neutral-controller attestation, live controller source, candidate Skill tree, fixture, and public aggregate are revalidated together at publication boundaries. See [ADR 0006](adr/0006-evidence-bound-publication-continuation.md) and [ADR 0011](adr/0011-exact-private-evaluation-binding.md).
